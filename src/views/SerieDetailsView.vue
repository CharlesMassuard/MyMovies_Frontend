...
const handleDateConfirm = async (dateData) => {
  try {
    ...
    if (dateData !== 'long-time-ago') {
      ...
      dateText = `Terminée le ${day}/${month}/${year}`;
    }
...
<template>
  <div v-if="serieDetails.name" class="serie-page">
    ...
    <!--Section-Épisodes-Dernier-et-Prochain-->
    ...
    <v-container class="mt-8" v-if="serieDetails.seasons?.length">
      ...
      <!--Liste-détaillée-des-épisodes-de-la-saison-->
      <v-expand-transition>
  ...
  <DateDialog
    v-model="dialogDate"
    title="Quand avez-vous fini cette série ?"
    @confirm="handleDateConfirm"
  />

  <RatingDialog
    v-model="dialogNote"
    title="Noter cette série"
